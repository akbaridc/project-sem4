...
<table class='table table-condensed'>
  <tbody>
    ...
    <tr>
      <th scope='row' width='100px'>Nama</th>
      <td><?php echo $rows['nama_reseller'] ?></td>
    </tr>
    <tr>
      <th scope='row'>Alamat</th>
      <td><?php echo "Kota $rows[nama_kota], $rows[alamat_lengkap]"; ?></td>
    </tr>
    <tr>
      <th scope='row'>Kontak</th>
      <td><?php echo "Hp. <b style='color:green'>$rows[no_telpon]</b>, Email. <i style='text-decoration:underline; color:blue'>$rows[email]</i>"; ?></td>
    </tr>
    <tr>
...
foreach ($record->result_array() as $row) {
  ...
  if ($beli['beli'] - $jual['jual'] <= 0) {
    ...
    $stok = "Stok " . ($beli['beli'] - $jual['jual']) . " $row[satuan]";
  }
  ...
  if ($disk['diskon'] == '') {
    ...
    $diskon = $disk['diskon'];
    $line = 'line-through';
    $harga = "/ <span style='color:red'>" . rupiah($row['harga'] - $disk['diskon']) . "</span>";
  }
