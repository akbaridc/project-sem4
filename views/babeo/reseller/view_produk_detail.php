...
if ($record['gambar'] != '') {
    ...
    for ($i = 0; $i < 1; $i++) {
        if (file_exists("asset/foto_produk/" . $ex[$i])) {
            if ($ex[$i] == '') {
                echo "<img style='height:120px; width:100%;  border:1px solid #cecece' src='" . base_url() . "asset/foto_produk/no.png'>";
            } else {
    ...
    for ($i = 1; $i < $hitungex; $i++) {
        if (file_exists("asset/foto_produk/" . $ex[$i])) {
            if ($ex[$i] == '') {
                echo "<img style='width:24%; border:1px solid #fff' src='" . base_url() . "asset/foto_produk/no.png'>";
            } else {
            ...
            echo "<img style='width:24%;  border:2px solid #fff' src='" . base_url() . "asset/foto_produk/no.png'>";
        }
    }
    echo "</center>";
} else {
...
$diskon = rupiah(($disk['diskon'] / $record['harga']) * 100, 0) . "%";
if ($disk['diskon'] > 0) {
...
if ($disk['diskon'] >= 1) {
    $harga =  "Rp " . rupiah($record['harga'] - $disk['diskon']);
    $harga_asli = "Rp " . rupiah($record['harga']);
} else {
    $harga =  "Rp " . rupiah($record['harga']);
    $harga_asli = "";
}

echo "<div style='clear:both'></div><center style='color:black;'><i>Klik untuk lihat ukuran besar.</i></center>
        </div>
...
echo "<table class='table table-condensed' style='margin-bottom:0px'>
                ...
                <h1 style='display:inline-block'>$harga</h1> / $record[satuan] 
...
if (($beli['beli'] - $jual['jual']) >= 1) {
    echo "<tr><td style='font-weight:bold'>Tersedia</td> <td style='color:red'>" . ($beli['beli'] - $jual['jual']) . " stok barang</td></tr>";
} else {
